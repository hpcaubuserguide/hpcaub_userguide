...
following specifications:
   ...
   - 880 cores
        ...
        + 72   miscellaneous CPUs and vCPUs for management / storage and other tasks
   - 3.0 TB main memory
...
.. todo:: the partition summary and the table below have not been verified
   against the current cluster. On the head node, check that every listed
   partition still exists and compare its time limit, node count and access
   with ``sinfo -s`` and ``scontrol show partition`` (``MaxTime``, ``Nodes``,
   ``AllowGroups``), and the cores, memory and GPUs of its nodes with
   ``scontrol show nodes`` (``CPUTot``, ``RealMemory``, ``Gres``). Leave out
   partitions that only accept the ``admin`` group.

The list below summarizes the main partitions:

  - ``normal``: 12 hosts with 16 vCPUs each with 64GB RAM.
  - ``gpu``: 12 hosts ( 8 with Nvidia K20m cards and 4 with Nvidia V100 cards).
